...
-- apaga outra tabela se existir chamada filmes
DROP TABLE IF EXISTS `filmes`;

-- criação da tabela com suas variáveis
CREATE TABLE filmes (
  id_filme INT, -- variável identificadora com tipagem número inteiro
  titulo VARCHAR(255) NOT NULL, -- limitação de caracteres em 255 - não existe filme sem título
  genero VARCHAR(100), -- limitação de caracteres em 100
  duracao INT, -- duração em minutos, por isso tipagem número inteiro
  ano_lancamento INT,
  preco_aluguel DECIMAL(5,2),  -- relação a preço com 2 casas decimais
  id_ator INT, -- variável para fk colocada abaixo
  PRIMARY KEY (id_filme),
  INDEX idx_fk_filmes_id_ator (id_ator)  -- otimização de busca nos join
) ENGINE=InnoDB -- mecanismo de armazenamento
DEFAULT CHARSET=utf8mb4 -- codificação usada para escrita
COLLATE=utf8mb4_0900_ai_ci; -- comparação não sensitiva para melhorar na busca relacional

LOCK TABLES `filmes` WRITE; -- comando para garantir única edição pelo desenvolvedor, minimizando riscos
/*!40000 ALTER TABLE `filmes` DISABLE KEYS */; -- otimização para inset em massa, evitando atualização de índice em cada insert
...
/*!40000 ALTER TABLE `filmes` ENABLE KEYS */;  -- depois do insert, atualização dos índices
UNLOCK TABLES; -- destrava a tabela 

ALTER TABLE filmes -- alteração da tabela enquanto estrutura
ADD CONSTRAINT fk_filmes_ator  -- integridade do banco
FOREIGN KEY (id_ator)
REFERENCES atores(id_ator)
ON UPDATE CASCADE -- atualização automática dos identifiadores nas tabelas
ON DELETE SET NULL -- caso seja deletado, atualizar o fk como nulo
